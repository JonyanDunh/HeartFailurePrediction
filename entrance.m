...
% Compute2(zscore(age),zscore(creatinine_phosphokinase),zscore(ejection_fraction),zscore(platelets),zscore(serum_creatinine),zscore(serum_sodium),zscore(time),DEATH_EVENT,theta,0.0015,100000);
Compute2(age,creatinine_phosphokinase./100,ejection_fraction,platelets./100000,serum_creatinine,serum_sodium./100,time,DEATH_EVENT,theta,0.0055,1);
%1000000
end
...
for s=1:iterationTimes
    for i=1:m
        ...
        data(timess,10)=Theta(8,1);
          fprintf('第%d次迭代\t 损失函数结果 %.5f\t Theta1 %.3f\t Theta2 %.3f\t Theta3 %.3f\t Theta4 %.3f\t Theta5 %.3f\t Theta6 %.3f\t Theta7 %.3f\t Theta8 %.3f\t \n',data(timess,1),data(timess,2),data(timess,3),data(timess,4),data(timess,5),data(timess,6),data(timess,7),data(timess,8),data(timess,9),data(timess,10))
        timess=timess+1;
    end
end
subplot(2,2,1)
